...
//			System.out.println((Integer)(node.getValue()) + " level: " + level + "  trueLevel: " + trueLevel + "  position: " + position);
	...
	{
		//this is the ArrayList that will hold the nodes in order
		ArrayList<Integer> nodes = new ArrayList<Integer>();
		traverse(original.getRoot(), nodes);
		balanced.setRoot(balance(nodes, 0, nodes.size()-1));
	}
	
	//this method goes through the unbalanced tree and gets the nodes in order
	//and puts them in the ArrayList
	public void traverse(TreeNode root, ArrayList<Integer> nodes)
	{
		if (root != null)
		{
			traverse(root.getLeft(), nodes);
			nodes.add((Integer)(root.getValue()));
			traverse(root.getRight(), nodes);
		}
	}
	
	//this is the recursive method that balances the binary search trees
	public TreeNode balance(ArrayList<Integer> nodes, int start, int end) 
	{ 
  
		//this is the base case of when the start value is greater than the end value
		if (start > end) 
		{ 
			return null; 
		} 
		...
		//Constructs a tree of the left side of the middle node
		node.setLeft(balance(nodes, start, mid - 1)); 

		//Constructs a tree of the right side of the middle node
		node.setRight(balance(nodes, mid + 1, end)); 
