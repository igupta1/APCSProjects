...
{ 
	...
	public void choosePath(int arrayChoice)
	{
		...
		while(curr < stones.length)
		{
			...
			while(canJump)
			{	
				boolean[] jumps = ableToJump(curr, jumpsPossible, stones, previous);
			...
			//repeats until the frog has exhausted all the jumps possible or until the frog is at the end of the river.
			for(int i = 1; curr + i < stones.length && i <= jumpsPossible; i++)
			{
				int counter = i + 1;
				
				//If the frog is at the end of the array or at the last rock, the final jump length is found
				if(curr + counter == stones.length)
				{
					if(stones[curr + i] - stones[curr] > maxLength)
					{
						max = i;
					}
				}
				...
				{
					int secondJump = stones[curr + counter] - stones[curr + i];
					int firstJump = stones[curr + i]  - stones[curr];
					
					while((firstJump <= 49 && secondJump < 71 || firstJump > 49 && secondJump <= 49) && (counter + curr + 1 < stones.length))
					{
						...
						counter++;
						secondJump = stones[curr + counter] - stones[curr + i];
					}
				}
			}
			...
			if(jumpsPossible == 0)
			{
				...
				previous = stones[curr - 1] - stones[first];

				
				if((stones[curr + 1] - stones[sec] <= 70 && previous < 50) || (stones[curr + 1] - stones[sec] < 50 && previous >= 50))
				{
					hops[mid] = false;
				}
			...
			previous = stones[curr + max] - stones[curr];
			curr += max;
			hops[curr] = true;
	...
	private boolean[] ableToJump(int currentIndex, int jumpsPossible, int[] stones, int previous)
	{
		...
		//checks if the target stone is less than 50 if it made a big jump in the last jump
		//or if the target stone is less than 70 if it made a small jump in the last jump.
		if(previous < 50 && target <= 70 || previous >= 50 && target < 50)
		{
			int next = currentIndex + jumpsPossible + 2;
			
			//checks if the next jump will put it at the end
			...
			{
				//gets the length of the second jump
